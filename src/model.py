...
class QuietStandingModel(object):
    ...
    G is a matrix of controller gains:

    G = [g_00, g_01, g_02, g_03]
        [g_10, g_11, g_12, g_13]

    but G is defined in terms of the scaling factor, S, and the scaled
    gains, K, to facilitate convergence when the model is used in an NLP
    optimization. K is chosen by passing a desired value into the
    initializer of this class.

    T = -S .* K * x          (.* is the Hadamard product)
    ...
    Introducing this relationship to the open loop equations are motion
    allows for the closed loop non-linear equations to be formed:
    ...
    def __init__(self, scaled_gains=None):
        """

        Parameters
        ==========
        scaled_gain : None or array_like, shape(2, 4)
            The desired numerical values of the scaled gains, K. The scaled
            gains are be Hadamard multiplied by the gain scale factors to
            get the actual gain matrix. If None, the gains are not scaled.
            For NLP problems, this should probably be between 0 and 1.

        """

        self.scaled_gains = scaled_gains
    ...
    def _create_specifieds(self):

        time_varying = [s(self.time)
                        for s in sy.symbols('x, v, a, T_h, T_a',
                                            cls=sy.Function, real=True)]

        self.specifieds = OrderedDict()
        self.specifieds['platform_position'] = time_varying[0]
        self.specifieds['platform_speed'] = time_varying[1]
        self.specifieds['platform_acceleration'] = time_varying[2]
        self.specifieds['ankle_torque'] = time_varying[3]
        self.specifieds['hip_torque'] = time_varying[4]
    ...
    def _set_positions(self):
        ...
        vec = self.specifieds['platform_position'] * self.frames['inertial'].x
    ...
    def _set_linear_velocities(self):
        ...
        self.points['ankle'].set_vel(self.frames['inertial'],
                                     self.specifieds['platform_speed'] *
    ...
    def _set_linear_accelerations(self):
        ...
        vec = (self.specifieds['platform_acceleration'] *
               self.frames['inertial'].x)
    ...
    def _create_loads(self):
        ...
        vec = (self.specifieds['ankle_torque'] * self.frames['inertial'].z -
               self.specifieds['hip_torque'] * self.frames['inertial'].z)
        self.loads['leg_torque'] = (self.frames['leg'], vec)

        self.loads['torso_torque'] = (self.frames['torso'],
                                      self.specifieds['hip_torque'] *
                                      self.frames['inertial'].z)

    def _setup_problem(self):
        self._create_states()
        self._create_specifieds()
    ...
    def _generate_eoms(self):
        ...
        # Manually replace v' with a.
        sub = {self.specifieds['platform_speed'].diff(self.time):
               self.specifieds['platform_acceleration']}

        #self.fr_plus_frstar = sy.trigsimp(fr + frstar).subs(sub)
        self.fr_plus_frstar = (fr + frstar).subs(sub)
    ...
    def _create_symbolic_controller(self):
        ...
        inputs = self.specifieds.values()[-2:]
    ...
    def _numerical_parameters(self):
        ...
        if self.scaled_gains is None:
            # In this case, K = G
            # S = [1, 1, 1, 1]
            #     [1, 1, 1, 1]
            self.gain_scale_factors = np.ones_like(self.numerical_gains)
        else:
            # Use the fact that the we know the correct numerical gains and
            # set S = G ./ K.
            self.gain_scale_factors = self.numerical_gains / self.scaled_gains
    ...
    def _linearize(self):
        ...
        states = self.coordinates.values() + self.speeds.values()
        # r = [T_a, T_h]
        specifieds = self.specifieds.values()[-2:]
        ...
        F_B = self.forcing_vector.jacobian(specifieds).subs(equilibrium)
